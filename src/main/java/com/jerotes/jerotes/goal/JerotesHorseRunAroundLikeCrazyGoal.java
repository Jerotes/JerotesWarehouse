...
public class JerotesHorseRunAroundLikeCrazyGoal extends Goal {
    private final JerotesHorseEntity horse;
    private final double speedModifier;
    private double posX;
    private double posY;
    private double posZ;

    public JerotesHorseRunAroundLikeCrazyGoal(JerotesHorseEntity p_25890_, double p_25891_) {
        this.horse = p_25890_;
    ...
    public void tick() {
        if (this.horse.getThrowingTick() > 0) {
            return;
        }
        if (!this.horse.isTamed() && this.horse.getRandom().nextInt(this.adjustedTickDelay(50)) == 0) {
            ...
            this.horse.makeMadOld();
            if (entity instanceof LivingEntity livingEntity) {
                if (!this.horse.level().isClientSide()) {
                    this.horse.setThrowingTick(11);
                    this.horse.setThrower(livingEntity);
                }
            }
        }
